...
class DirectProcessing(BaseProcessing):
    """ The processing class used for training ATOM. The images are processed in the following way.
    First, the target bounding box is jittered by adding some noise. Next, a square region (called search region )
    centered at the jittered target center, and of area search_area_factor^2 times the area of the jittered box is
    cropped from the image. The reason for jittering the target box is to avoid learning the bias that the target is
    ...
    def _generate_proposals(self, box):
        ...
        num_proposals = self.proposal_params['boxes_per_frame']
        proposals = torch.zeros((num_proposals, 4))
        regs = torch.zeros(num_proposals,4)

        for i in range(num_proposals):
            proposals[i, :], regs[i,:] = prutils.perturb_box2(box, min_iou=self.proposal_params['min_iou'],
                                                             sigma_factor=self.proposal_params['sigma_factor']
                                                             )

        # Map to [-1, 1]
        # gt_iou = gt_iou * 2 - 1
        return proposals, regs
    ...
    def __call__(self, data: TensorDict):
        ...
        # Generate proposals
        frame2_proposals, regs = zip(*[self._generate_proposals(a) for a in data['test_anno']])
